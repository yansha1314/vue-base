<template>
  <div class="post-list">
    <!--这个div用来避免这个组件成为片段实例-->
    <article v-for="post in lists">
      <header>
        <h2><a v-link="'/posts/'+post['id']">{{post['askEntName']}}</a></h2>
        <h4>
          {{post['bidData']}}
        </h4>
      </header>
      <footer>
        <a v-link="'/posts/'+post['id']">... continue reading</a>
      </footer>
...
<script>
  ...
  import service from '../services/list/index'
